...
class TimescaleVector(VectorStore):
    """VectorStore implementation using the timescale vector client to store vectores in Postgres.
    ...
    def create_index(
        ...
        if (index_type == self.IndexType.TIMESCALE_VECTOR):
            self.sync_client.create_embedding_index(client.TimescaleVectorIndex(**kwargs))
